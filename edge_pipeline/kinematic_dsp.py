...
class KinematicPhysicsEngine: 
    def __init__(self, sample_rate, rpm, bearing_geometry):
        self.fs = sample_rate
    ...
    def _calculate_theoretical_frequencies(self):
        """Calculates exact physical defect frequencies for ALL known faults."""
        ...
        # High-Frequency Bearing Kinematics (The Complete Matrix)
        ...
        # Low-Frequency Rotational Harmonics
    ...
    def _raw_spectrum(self, raw_signal):
        """Standard FFT for low-frequency macro faults (Unbalance/Misalignment)."""
        n = len(raw_signal)
        yf = np.abs(rfft(raw_signal))
        xf = rfftfreq(n, 1 / self.fs)
        return xf, yf

    def _envelope_spectrum(self, raw_signal):
        """Envelope Analysis for high-frequency micro-impacts (Bearings)."""
        nyq = 0.5 * self.fs
        b, a = butter(4, [2000 / nyq, 4000 / nyq], btype='band')
        filtered_sig = filtfilt(b, a, raw_signal)
        analytic_signal = hilbert(filtered_sig)
        amplitude_envelope = np.abs(analytic_signal)
        n = len(amplitude_envelope)
        yf = np.abs(rfft(amplitude_envelope))
        xf = rfftfreq(n, 1 / self.fs)
        return xf, yf

    def analyze_faults(self, raw_signal, tolerance_hz=2.0):
        """Analyzes both frequency domains and returns strict boolean logic."""
        # 1. Check Envelope Spectrum for Bearings
        ...
        # 2. Check Raw Spectrum for Rotational Harmonics
        ...
        # Deterministic Boolean Matching (Fully Restored)
        fault_flags = {
...
# ONE-LINE SUMMARY: The unified physics engine now successfully checks both raw and envelope spectrums against the complete library of 6 critical mechanical faults.